  ...
  def self.create_with_photo(user, tripnote, photo, body: nil)
    ...
    if photo.image_lat && photo.image_lng
      begin
        ...
        # NOTE: known issue with (lat,lng) = (0.0,0.0): Foursquare2::APIError Exception: param_error: Must provide parameter ll or userll or near (400)
        logger.error ex.message
        fsq_results = nil
      end
      if fsq_results and fsq_results.venues.length > 0
        # NOTE: we use first one of Fsq result (trusting Fsq order by populality)
